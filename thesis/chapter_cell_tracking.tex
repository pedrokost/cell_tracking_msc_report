    ...
    \section{Computing the likelihoods \statusfirstdraft}
     	...
     	\subsubsection{The linking hypothesis \statusfirstdraft}
       	...
       	\subsubsection{The initialization likelihood \statusfirstdraft}
       		...
       		This assumes that the initialization likelihood is independent of the status of any of the previous tracklets. For example, if all of the previous tracklets are selected by the algorithm to be false positives, then $P_{init}(T_k)$ should be increased. Although this assumption limits the model, it is required to limit the number of hypothesis that need to be computed. The same assumption is made for computing the termination hypothesis.
		...
		\subsubsection{Scaling hypothesis' likelihoods \statusfirstdraft}
		\label{sec:tracking_scalingparameters}
		Each of the hypothesis likelihoods can be scaled by an appropriate parameter $\pi_{FP}$, $\pi_{link}$, $\pi_{init}$ or $\pi_{term}$. The adjustment of these parameters allows a direct manipulation of the hypotheses. For example, increasing $\pi_{init}$ or $\pi_{term}$ relative to $\pi_{link}$ makes the system more conservative in linking tracklets. The added benefit of these scaling parameters is that they allow the reuse of the trained classifier for somewhat different datasets, without the need to annotate them and retrain the classifier.
 	...
 	\section{Features for the linking classifier \statusoutline}
    	...
    	\end{description}
 	    	...
 	    	\todo[inline]{Describe which features performed best... and figure out where I place the plot of feauture selection and link it here}
	  ...
	  \section{Implementation details \statusfirstdraft}
